...
def get_goal_spec(game: str):
    """
    Return the known liveness goal specification for a game as an Op.

    These use symbolic constant names (goalx, goaly, etc.) that are
    defined in the TSLMT spec generated by the synthesis pipeline.
    """
    ...
    if game in ("frozen_lake", "ice_lake"):
        ...
        # F (eqC player locR && F (eqC player destination))
        # Taxi has a two-phase goal: reach pickup then reach dropoff
        eq_pickup = Predicate.from_string("eqC player locR")
        eq_dropoff = Predicate.from_string("eqC player destination")
        return Eventually(And(eq_pickup, Eventually(eq_dropoff)))
