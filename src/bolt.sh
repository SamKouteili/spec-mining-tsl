...
if [[ $# -ne 3 ]]; then
  echo "Usage: $0 <logs_dir> <metadata.py> <output_dir>" >&2
  exit 1
fi
...
if [[ ! -f "$PROCESS_PY" ]]; then
  echo "[bolt.sh] Error: process.py not found at $PROCESS_PY" >&2
  exit 1
fi
...
echo "[bolt.sh] Generating BOLT traces with process.py"
python "$PROCESS_PY" "$LOGS_DIR" "$META_FILE" --out "$OUT_DIR"
