#!/usr/bin/env python3
"""
LTL Bit-Blasting Baseline for CliffWalking TSL_f Specification Mining Evaluation
...
For cliff_walking:
    Variables (integers, need 4 bits each for values 0-15):
        - x, y (player position)
        - goalX, goalY (goal position)
        - cliffXMin, cliffXMax, cliffHeight (cliff parameters)

    Total bits: 7 variables * 4 bits = 28 atomic propositions + END = 29
"""
...
# Number of bits needed to represent values 0-15 (sufficient for x up to 12, y up to 4)
NUM_BITS = 4
MAX_VALUE = 15
...
def convert_traces_to_bitblasted(trace_dir: Path, game: str = "cliff_walking") -> Dict[str, List[Dict[str, List[int]]]]:
    """
    Convert JSONL traces to bit-blasted Boolean traces.

    Each integer variable is converted to NUM_BITS individual Boolean APs.
    For example, x=3 becomes:
        x_b0=1, x_b1=1, x_b2=0, x_b3=0 (since 3 = 0b0011)

    Args:
        trace_dir: Directory containing pos/ and neg/ subdirectories with JSONL files
        game: Game type (only "cliff_walking" supported in this module)

    Returns:
        Dict with "positive_traces" and "negative_traces" lists
    """
    if game != "cliff_walking":
        raise NotImplementedError(f"Bit-blasting baseline not implemented for {game} in this module")
...
def _convert_single_trace_bitblasted(trace_file: Path, game: str) -> Optional[Dict[str, List[int]]]:
    ...
    """
    if game != "cliff_walking":
        raise NotImplementedError(f"Bit-blasting baseline not implemented for {game}")
    ...
    # Define variables to bit-blast for cliff_walking
    # State format: {"x": int, "y": int, "goalX": int, "goalY": int,
    #                "cliffXMin": int, "cliffXMax": int, "cliffHeight": int}
    variables = [
        "x",
        "y",
        "goalX",
...
def create_bolt_file(bitblasted_traces: Dict[str, List[Dict[str, List[int]]]], output_path: Path) -> Path:
    ...
    # This gives a sensible ordering like: x_b0, x_b1, ..., y_b0, ...
    def ap_sort_key(ap):
        if ap == "END":
...
def interpret_bitblasted_spec(
    ...
    1. Detect patterns like "all bits of x equal all bits of goalX"
    ...
    bit_refs = []
    variables = ["x", "y", "goalX", "goalY", "cliffXMin", "cliffXMax", "cliffHeight"]
    ...
    # Check for x == goalX && y == goalY pattern (reaching goal)
    if liveness:
        if check_equality_pattern(liveness, "x", "goalX") and check_equality_pattern(liveness, "y", "goalY"):
    ...
    if safety:
        # Check for cliff avoidance patterns
        has_cliff_ref = False
        for var in ["cliffXMin", "cliffXMax", "cliffHeight"]:
            for bit in range(NUM_BITS):
                if f"{var}_b{bit}" in safety:
                    has_cliff_ref = True
                    break

        if has_cliff_ref:
            interpretation_notes.append("Safety mentions cliff bits - potential cliff avoidance, but bit-level")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="LTL Bit-Blasting Baseline for CliffWalking TSL_f Evaluation",
...
"""
    ...
    parser.add_argument("--game", "-g", type=str, default="cliff_walking",
                        choices=["cliff_walking"],
                        help="Game type (default: cliff_walking)")
