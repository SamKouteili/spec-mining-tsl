#!/usr/bin/env python3
"""
Decision Tree (DT) Baseline for FrozenLake
...
def train(trace_dir: Path, max_depth: int = 20,
          random_state: int = 42, deduplicate: bool = False) -> Tuple[DecisionTreeClassifier, float, dict]:
    """
...
def train_and_evaluate(trace_dir: Path, test_configs: List[dict],
    ...
    start = time.time()
    model, train_acc, meta = train(trace_dir, max_depth=max_depth)
